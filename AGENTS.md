...
- **False Observer claim removed (RCA-066)**: `SeatMapNotifier` iterated an observer list that no
  production code ever populated; `SeatAvailabilityObserver` had zero implementations. Both dead
  types and the notifier parameters threaded through `MovieTicketService`/`SeatLockManager` were
  removed, along with README/design/diagram Observer claims. The module's real runtime patterns
  remain pricing Strategy + Factory and seat creation Factory.
...
- **Read-side consistency (RCA-067)**: `Wallet.balance` remains a plain lock-guarded `double`, so every live and sim balance read now acquires the same per-wallet lock as writers. `getAllWallets`/sim snapshots acquire every wallet lock in ascending id order (the same order as `TransferCommand`) and return detached `Wallet` copies, preventing both a half-transfer aggregate and post-return JSON serialization racing a later mutation. Mutation responses use balances captured by each command while its lock(s) were still held; `WalletRepository` exposes sorted ids, not an unsafe unlocked `totalBalance()`/live-object list.
...
- Tests (new, previously zero): `WalletRepositoryTest` (seed data, sorted ids, atomic id generation, transaction filing under one vs. both wallets), `WalletCommandTest` (each command's validation/arithmetic and lock-captured result balances in isolation — insufficient balance, non-positive amount, unknown wallet, self-transfer, and that a rejected command leaves balances unchanged), `WalletServiceTest` (service-level orchestration, detached read snapshots, the command log recording exactly the executed commands and never a rejected one, sim-sandbox isolation from live balances), `WalletConcurrencyTest` (the load-bearing suite — 40 simultaneous transfers between two wallets latch-started together assert the combined balance is conserved *exactly*; 200 rounds of opposite-direction transfers racing the same pair prove no deadlock and exact conservation; disjoint wallet pairs transferring concurrently don't corrupt each other; a 100-round repeated head-to-head race never loses or creates money; a paused in-flight write proves `getBalance` waits on the writer lock; a paused half-transfer proves aggregate snapshots wait for both sides). All race starts/critical pauses are latch-gated, not sleep-based.
...
cd frontend && npx vitest run # 444 tests, 10 files
```

...
| **5 — sequence diagrams** | ~~every module touched above (+ sweep of the solid tier)~~ | Done — all 45 modules have `data/sequences/<module>.js`. Added in one un-reviewed direct push to `main` (`2f1d52e`, no branch/PR/CI — a real process violation, see RCA-030), then found to be substantially fabricated in three passes: RCA-030 (`atm`, `library`, `shoppingcart` severely fabricated; `hotel`, `cricinfo`†, `course-registration`, `music-streaming`, `car-rental`, `stackoverflow`, `blocking-queue` had smaller naming errors), RCA-031 (`stock-brokerage`, which evaded RCA-030's own detection grep via embedded newlines), and RCA-034 (the 8 files RCA-030 never sampled at all — `bloom-filter`, `concurrent-hashmap`, `fizz-buzz`, `foo-bar`, `h2o`, `merge-sort`, `zero-even-odd`, `ttl-cache` — plus `cricinfo`† and `logging-framework`, which RCA-030's original spot-check had wrongly marked clean). Every one of the 45 files has now been individually read against real source at least once — no unsampled tier remains. |

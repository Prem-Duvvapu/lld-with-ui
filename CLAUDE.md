...
what each of the 53 modules seeds, which service methods exist, which patterns it demonstrates.
...
mvn test                                  # full suite — 2039 tests, 235 classes
...
npx vitest run                            # full suite — 352 tests, 3 files
...
strategy / exception / config` sub-packages. `LldApplication` boots all 45 module packages at once
(`concurrency` nests nine primitive sub-packages of its own: `blockingqueue`, `bloomfilter`,
`concurrenthashmap`, `fizzbuzz`, `foobar`, `h2o`, `mergesort`, `ttlcache`, `zeroevenodd` — 44
standalone modules + those 9 concurrency primitives = the 53 LLD problems the frontend exposes as
individual pages).
...
All 53 modules now have backends — the last three concurrency primitives (`bloom-filter`,

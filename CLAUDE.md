...
what each of the 57 modules seeds, which service methods exist, which patterns it demonstrates.
...
mvn test                                  # full suite — 2164 tests, 256 classes
...
npx vitest run                            # full suite — 376 tests, 3 files
...
strategy / exception / config` sub-packages. `LldApplication` boots all 49 module packages at once
(`concurrency` nests nine primitive sub-packages of its own: `blockingqueue`, `bloomfilter`,
`concurrenthashmap`, `fizzbuzz`, `foobar`, `h2o`, `mergesort`, `ttlcache`, `zeroevenodd` — 48
standalone modules + those 9 concurrency primitives = the 57 LLD problems the frontend exposes as
individual pages).
...
All 57 modules now have backends — the last three concurrency primitives (`bloom-filter`,

...
what each of the 58 modules seeds, which service methods exist, which patterns it demonstrates.
...
mvn test                                  # full suite — 2192 tests, 261 classes
...
npx vitest run                            # full suite — 382 tests, 3 files
...
strategy / exception / config` sub-packages. `LldApplication` boots all 50 module packages at once
(`concurrency` nests nine primitive sub-packages of its own: `blockingqueue`, `bloomfilter`,
`concurrenthashmap`, `fizzbuzz`, `foobar`, `h2o`, `mergesort`, `ttlcache`, `zeroevenodd` — 49
standalone modules + those 9 concurrency primitives = the 58 LLD problems the frontend exposes as
individual pages).
...
All 58 modules now have backends — the last three concurrency primitives (`bloom-filter`,

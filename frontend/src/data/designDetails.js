    ...
    entities: [
      {
        ...
        fields: [
          { name: 'machineId', type: 'String', description: 'Unique hardware machine identifier (e.g. VM-PROD-01)' },
          { name: 'currentState', type: 'VendingMachineState', description: 'Active State Pattern instance (IdleState, HasSelectionState, HasMoneyState, DispensingState)' },
          { name: 'slots', type: 'Map<String, Slot>', description: 'ConcurrentHashMap of 12 slots indexed by slot code (A1-C4)' },
          { name: 'changeInventory', type: 'Map<Denomination, AtomicInteger>', description: 'Hopper coin and banknote inventory per denomination' },
          { name: 'lock', type: 'ReentrantLock', description: 'Guarantees sequential state transitions and race-free inventory mutations' },
          { name: 'changeDispenserChain', type: 'ChangeDispenserChain', description: 'Chain of Responsibility handler pipeline for change calculation' },
        ],
        methods: [
          { name: 'selectProduct(slotCode)', returns: 'void', description: 'Delegates to currentState.selectProduct(this, slotCode)' },
          { name: 'insertMoney(denomination)', returns: 'void', description: 'Delegates to currentState.insertMoney(this, denomination)' },
          { name: 'dispense()', returns: 'Transaction', description: 'Delegates to currentState.dispense(this)' },
          { name: 'cancelTransaction()', returns: 'Transaction', description: 'Delegates to currentState.cancelTransaction(this)' },
          { name: 'refillChange(denomination, count)', returns: 'void', description: 'Restocks coin/bill hopper with thread safety' },
        ],
      },
      {
        ...
        fields: [],
        methods: [
          { name: 'selectProduct(machine, slotCode)', returns: 'void', description: 'Handles product selection for the given state' },
          { name: 'insertMoney(machine, denomination)', returns: 'void', description: 'Handles cash insertion for the given state' },
          { name: 'dispense(machine)', returns: 'Transaction', description: 'Handles item dispensing and change payout' },
          { name: 'cancelTransaction(machine)', returns: 'Transaction', description: 'Handles cancellation and refund calculation' },
          { name: 'getStatus()', returns: 'MachineStatus', description: 'Returns high-level status enum' },
        ],
      },
      {
        ...
        fields: [
          { name: 'nextHandler', type: 'ChangeDispenseHandler', description: 'Next denomination handler in the descending pipeline' },
          { name: 'denomination', type: 'Denomination', description: 'Specific coin/note handled (e.g. ₹500, ₹100, ₹50...)' },
        ],
        methods: [
          { name: 'setNext(handler)', returns: 'ChangeDispenseHandler', description: 'Links next handler in pipeline' },
          { name: 'handle(remaining, available, dispensed)', returns: 'void', description: 'Calculates maximum units dispensable from available inventory and passes remainder' },
        ],
      },
      {
        name: 'Slot',
        ...
        fields: [
          { name: 'code', type: 'String', description: 'Grid code (A1-A4, B1-B4, C1-C4)' },
          { name: 'row / col', type: 'int', description: 'Matrix coordinates' },
          { name: 'product', type: 'Product', description: 'Assigned product metadata' },
          { name: 'capacity', type: 'int', description: 'Max capacity per slot (10 items)' },
          { name: 'currentStock', type: 'int', description: 'Current available stock count' },
        ],
        methods: [
          { name: 'isAvailable()', returns: 'boolean', description: 'True if currentStock > 0 and product != null' },
          { name: 'decrementStock()', returns: 'void', description: 'Decrements stock count by 1 upon dispense' },
          { name: 'restock(amount)', returns: 'void', description: 'Adds inventory up to max capacity' },
        ],
      },
      {
        name: 'Transaction',
        ...
        fields: [
          ...
          { name: 'slotCode', type: 'String', description: 'Target slot code' },
          { name: 'itemPrice', type: 'double', description: 'Target price in INR' },
          { name: 'insertedAmount', type: 'double', description: 'Cumulative money deposited' },
          { name: 'changeAmount', type: 'double', description: 'Change returned to customer' },
          { name: 'changeBreakdown', type: 'Map<String, Integer>', description: 'Denomination counts returned (e.g. 1x ₹10, 1x ₹5)' },
          { name: 'status', type: 'String', description: 'PENDING, PAID, DISPENSED, REFUNDED, FAILED' },
        ],
        methods: [],
      },
    ],
    ...
    entities: [
      ...
      },
      {
        ...
        fields: [
          ...
          { name: 'enabled', type: 'boolean', description: 'Active toggle state' },
        ],
        methods: [
          { name: 'append(msg, formatter)', returns: 'void', description: 'Writes formatted log message to target sink' },
          { name: 'getStatus()', returns: 'AppenderStatus', description: 'Returns telemetry status including file size and rotation counts' },
        ],
      },
      {
        ...
        fields: [],
        methods: [
          { name: 'format(msg)', returns: 'String', description: 'Formats LogMessage into output string' },
          { name: 'getType()', returns: 'FormatterType', description: 'Returns FormatterType enum' },
        ],
      },
      {
        ...
        fields: [
          { name: 'queue', type: 'BlockingQueue<LogTask>', description: 'Bounded buffer queue' },
          { name: 'droppedCount', type: 'AtomicLong', description: 'Counter tracking overflow dropped logs' },
        ],
        methods: [
          { name: 'dispatch(msg, appenders, formatter)', returns: 'boolean', description: 'Enqueues task into queue or increments drop counter if full' },
        ],
      },
    ],
    designPatterns: [
      { name: 'Chain of Responsibility', used: true, explanation: 'LogHandler chain (Trace -> Debug -> Info -> Warn -> Error -> Fatal) dynamically filters log events based on active threshold before passing to appenders.' },
      { name: 'Strategy Pattern (Formatters)', used: true, explanation: 'LogFormatter interface implemented by SimpleTextFormatter, JsonFormatter, and PatternFormatter, resolved dynamically via LogFormatterFactory.' },
      { name: 'Observer Pattern (Appenders)', used: true, explanation: 'Loggers broadcast log events to all attached LogAppender sinks (Console, File, Database, Elasticsearch) acting as event observers.' },
      { name: 'Decorator / Producer-Consumer', used: true, explanation: 'AsyncLogDispatcher wraps appender execution asynchronously using a bounded ArrayBlockingQueue and worker thread.' },
      { name: 'Singleton / Registry', used: true, explanation: 'LogManager acts as a thread-safe central logger registry managing logger hierarchy and root logger configuration.' },
    ],
    solidPrinciples: [
      { name: 'Single Responsibility (SRP)', description: 'Loggers manage level checking; LogHandlers manage chain filtering; LogFormatters handle text layout; LogAppenders handle sink storage.' },
      { name: 'Open/Closed (OCP)', description: 'New appender types (e.g., KafkaAppender, SlackAppender) or formatters (e.g., XmlFormatter) can be added without modifying existing logger code.' },
      { name: 'Liskov Substitution (LSP)', description: 'All LogAppenders (Console, File, DB, ES) can be interchanged seamlessly via the LogAppender interface.' },
      { name: 'Interface Segregation (ISP)', description: 'LogAppender and LogFormatter interfaces are concise and focused on a single responsibility.' },
      { name: 'Dependency Inversion (DIP)', description: 'Logger depends on LogAppender and LogFormatter interfaces rather than concrete implementations.' },
    ],
    concepts: [
      { name: 'Composition over Inheritance', description: 'Logger has-a List of Appender, not is-a Appender. LoggingFramework has-a root Logger and List of Appender. Behavior is assembled, not inherited.', alternative: 'Could use inheritance for specialized loggers. Composition is chosen because loggers differ in configuration, not behavior.' },
    ],
    extensibility: [
      { area: 'New Appender Destinations', description: 'Implement LogAppender (e.g. CloudWatchAppender, KafkaAppender) and add to LogManager.', difficulty: 'Easy' },
      { area: 'Custom Pattern Formatters', description: 'Implement LogFormatter to support custom log format tokens like %X{userId}.', difficulty: 'Easy' },
      { area: 'Dynamic Level Watcher', description: 'Reload application log configuration dynamically from external config server without restart.', difficulty: 'Medium' },
    ],
  },
    ...
    entities: [
      ...
      },
      {
        ...
        fields: [
          ...
          { name: 'type', type: 'ExpenseEventType', description: 'Event category enum: USER_CREATED, GROUP_CREATED, MEMBER_ADDED, EXPENSE_ADDED, SETTLEMENT' },
      ...
      },
      {
        ...
        fields: [
          { name: 'USER_CREATED', type: 'EnumConstant', description: 'Emitted when a new user joins the platform' },
          { name: 'GROUP_CREATED', type: 'EnumConstant', description: 'Emitted when an expense group is created' },
          { name: 'MEMBER_ADDED', type: 'EnumConstant', description: 'Emitted when a user is added to an existing group' },
          { name: 'EXPENSE_ADDED', type: 'EnumConstant', description: 'Emitted when a bill is added and split among participants' },
          { name: 'SETTLEMENT', type: 'EnumConstant', description: 'Emitted when a balance debt is settled between two members' },
        ],
        methods: [],
      },
      {

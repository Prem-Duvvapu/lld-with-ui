const classDiagrams = {
  shoppingcart: {
    ...
    classes: [
      { name: 'ShoppingCartService', stereotype: 'singleton', fields: ['- products: Map<String, Product>', '- carts: Map<String, Cart>', '- orders: Map<String, Order>', '- userCommandHistory: Map<String, Stack<CartCommand>>'], methods: ['+ addProduct(product): void', '+ addToCart(userId, productId, quantity): void', '+ undoLastCartCommand(userId): boolean', '+ placeOrder(userId, method, idempKey): Order', '+ cancelOrder(orderId): void'] },
      { name: 'Product', fields: ['- id: String', '- name: String', '- price: double', '- stockQuantity: AtomicInteger', '- productLock: ReentrantLock'], methods: ['+ decrementStock(qty): boolean', '+ incrementStock(qty): void'] },
      { name: 'Cart', fields: ['- userId: String', '- items: Map<String, CartItem>'], methods: ['+ addItem(product, qty): void', '+ removeItem(productId): void', '+ updateQuantity(productId, qty): void', '+ getTotalAmount(): double'] },
      { name: 'Order', fields: ['- orderId: String', '- userId: String', '- items: List<OrderItem>', '- totalAmount: double', '- status: OrderStatus', '- paymentMethod: PaymentMethod'], methods: ['+ setStatus(status): void'] },
      { name: 'CartCommand', stereotype: 'interface', fields: [], methods: ['+ execute(): void', '+ undo(): void'] },
      { name: 'AddItemCommand', fields: ['implements CartCommand', '- cart: Cart', '- product: Product', '- quantity: int'], methods: ['+ execute(): void', '+ undo(): void'] },
      { name: 'RemoveItemCommand', fields: ['implements CartCommand', '- cart: Cart', '- removedItem: CartItem'], methods: ['+ execute(): void', '+ undo(): void'] },
      { name: 'ShoppingCartPaymentProcessor', stereotype: 'strategy-router', fields: ['- strategies: Map<PaymentMethod, PaymentStrategy>'], methods: ['+ executePayment(orderId, amount, method): String'] },
      { name: 'PaymentStrategy', stereotype: 'interface', fields: [], methods: ['+ processPayment(orderId, amount): String', '+ getMethod(): PaymentMethod'] },
    ],
    relationships: [
      ...
      { from: 'ShoppingCartService', to: 'CartCommand', label: 'executes & undoes' },
      ...
      { from: 'ShoppingCartService', to: 'ShoppingCartPaymentProcessor', label: 'delegates payments to' },
    ]
  },
  pubsub: {
    title: 'Pub/Sub System (Message Broker) — Class Diagram',
    classes: [
      { name: 'PubSubService', stereotype: 'singleton', fields: ['- broker: Broker', '- activeSubscribers: Map<String, Subscriber>'], methods: ['+ createTopic(name): Topic', '+ subscribe(topic, subId, name, type, cap, delay): void', '+ unsubscribe(topic, subId): void', '+ publish(topic, payload, pubId): List<String>', '+ simPublish(topic, payload, pubId): List<TopicSnapshot>'] },
      { name: 'Broker', fields: ['- topics: ConcurrentHashMap<String, Topic>', '- messageIdGen: AtomicLong'], methods: ['+ createTopic(name): void', '+ getTopic(name): Topic', '+ subscribe(topic, sub, cap): void', '+ unsubscribe(topic, subId): void', '+ publish(topic, payload, pubId, headers): List<String>', '+ shutdown(): void'] },
      { name: 'Topic', fields: ['- name: String', '- workers: CopyOnWriteArrayList<SubscriberWorker>', '- publishedCount: AtomicLong'], methods: ['+ addSubscriber(sub, cap): void', '+ removeSubscriber(subId): void', '+ publish(message): List<String>'] },
      { name: 'SubscriberWorker', stereotype: 'runnable', fields: ['- subscriber: Subscriber', '- queue: ArrayBlockingQueue<Message>', '- queueCapacity: int', '- deliveredCount: AtomicLong', '- rejectedCount: AtomicLong', '- workerThread: Thread'], methods: ['+ enqueue(message): boolean', '+ run(): void', '+ stopGracefully(): void'] },
      { name: 'Subscriber', stereotype: 'interface', fields: [], methods: ['+ getId(): String', '+ getName(): String', '+ consume(message): void'] },
      { name: 'PrintSubscriber', fields: ['implements Subscriber', '- receivedMessages: CopyOnWriteArrayList<Message>'], methods: ['+ consume(message): void'] },
      { name: 'LoggingSubscriber', fields: ['implements Subscriber', '- logs: CopyOnWriteArrayList<String>'], methods: ['+ consume(message): void'] },
      { name: 'SlowSubscriber', fields: ['implements Subscriber', '- processDelayMs: long', '- receivedMessages: CopyOnWriteArrayList<Message>'], methods: ['+ consume(message): void'] },
      { name: 'Message', fields: ['- id: String', '- topicName: String', '- payload: String', '- publisherId: String', '- timestampEpoch: long'], methods: [] },
    ],
    relationships: [
      { from: 'PubSubService', to: 'Broker', label: 'wraps' },
      { from: 'Broker', to: 'Topic', label: 'manages' },
      { from: 'Topic', to: 'SubscriberWorker', label: 'dispatches to' },
      { from: 'SubscriberWorker', to: 'Subscriber', label: 'drains to' },
      { from: 'PrintSubscriber', to: 'Subscriber', label: 'implements', dashed: true },
      { from: 'LoggingSubscriber', to: 'Subscriber', label: 'implements', dashed: true },
      { from: 'SlowSubscriber', to: 'Subscriber', label: 'implements', dashed: true },
      { from: 'SubscriberWorker', to: 'Message', label: 'enqueues & processes' },
    ]
  },
  parking: {
  ...
  zomato: {
    title: 'Zomato Food Delivery — Class Diagram',
    classes: [
      { name: 'ZomatoService', stereotype: 'singleton', fields: ['- repository: ZomatoRepository'], methods: ['+ registerCustomer(name, email, phone, addr): Customer', '+ getRestaurants(): List<Restaurant>', '+ updateMenuItemAvailability(restId, itemId, avail): Restaurant', '+ placeOrder(custId, restId, items, addr, payMethod): Order', '+ confirmOrder(orderId): Order', '+ startPreparingOrder(orderId): Order', '+ markReadyForPickup(orderId): Order', '+ verifyOtpAndDeliver(orderId, otp): Order', '+ cancelOrder(orderId, reason): Order'] },
      { name: 'Customer', fields: ['- id: String', '- name: String', '- email: String', '- phone: String', '- deliveryAddress: String'], methods: ['+ getId()', '+ getDeliveryAddress()'] },
      { name: 'Restaurant', fields: ['- id: String', '- name: String', '- address: String', '- cuisine: String', '- rating: double', '- open: boolean', '- menu: List<MenuItem>'], methods: ['+ addMenuItem(item): void', '+ removeMenuItem(itemId): void', '+ getMenuItem(itemId): MenuItem'] },
      { name: 'MenuItem', fields: ['- id: String', '- name: String', '- description: String', '- price: double', '- category: String', '- isVeg: boolean', '- available: boolean'], methods: ['+ isAvailable(): boolean', '+ setAvailable(b): void'] },
      { name: 'Order', fields: ['- id: String', '- customerId: String', '- restaurantId: String', '- items: List<OrderItem>', '- itemTotal: double', '- deliveryFee: double', '- tax: double', '- totalAmount: double', '- status: OrderStatus', '- deliveryAgentId: String', '- payment: Payment', '- deliveryOtp: String'], methods: ['+ setStatus(status): void', '+ setDeliveryAgentId(id): void'] },
      { name: 'OrderItem', fields: ['- itemId: String', '- name: String', '- price: double', '- quantity: int', '- specialInstructions: String'], methods: ['+ getPrice()', '+ getQuantity()'] },
      { name: 'DeliveryAgent', fields: ['- id: String', '- name: String', '- phone: String', '- vehicleNumber: String', '- available: boolean', '- totalDeliveries: int'], methods: ['+ isAvailable(): boolean', '+ incrementDeliveries(): void'] },
      { name: 'Payment', fields: ['- id: String', '- orderId: String', '- amount: double', '- paymentMethod: PaymentMethod', '- status: PaymentStatus', '- transactionRef: String'], methods: ['+ setStatus(status): void'] },
      { name: 'Notification', fields: ['- id: String', '- recipientType: String', '- recipientId: String', '- orderId: String', '- message: String', '- timestamp: LocalDateTime'], methods: [] },
      ...
      { name: 'PaymentMethod', stereotype: 'enum', fields: ['UPI', 'CREDIT_CARD', 'DEBIT_CARD', 'CASH_ON_DELIVERY', 'WALLET'], methods: [] },
      { name: 'PaymentStatus', stereotype: 'enum', fields: ['PENDING', 'COMPLETED', 'FAILED', 'REFUNDED'], methods: [] },
      { name: 'ZomatoRepository', fields: ['- customers: ConcurrentHashMap', '- restaurants: ConcurrentHashMap', '- deliveryAgents: ConcurrentHashMap', '- orders: ConcurrentHashMap', '- notifications: SynchronizedList', '- lock: ReentrantLock'], methods: ['+ saveOrder(order): void', '+ getOrdersByCustomer(id): List', '+ getAvailableDeliveryAgents(): List'] },
    ],
    relationships: [
      { from: 'ZomatoService', to: 'ZomatoRepository', label: 'uses' },
      { from: 'ZomatoService', to: 'Order', label: 'manages lifecycle' },
      { from: 'ZomatoRepository', to: 'Customer', label: 'stores' },
      { from: 'ZomatoRepository', to: 'Restaurant', label: 'stores' },
      { from: 'ZomatoRepository', to: 'DeliveryAgent', label: 'stores' },
      { from: 'ZomatoRepository', to: 'Order', label: 'stores & locks' },
      { from: 'Order', to: 'Customer', label: 'placed by' },
      { from: 'Order', to: 'Restaurant', label: 'placed at' },
      { from: 'Order', to: 'OrderItem', label: 'contains list' },
      { from: 'OrderItem', to: 'MenuItem', label: 'references' },
      { from: 'Restaurant', to: 'MenuItem', label: 'has menu items' },
      { from: 'Order', to: 'DeliveryAgent', label: 'assigned to' },
      { from: 'Order', to: 'Payment', label: 'has payment' },
      { from: 'Order', to: 'OrderStatus', label: 'has status' },
      { from: 'Payment', to: 'PaymentMethod', label: 'uses method' },
      { from: 'Payment', to: 'PaymentStatus', label: 'has status' },
      { from: 'ZomatoService', to: 'Notification', label: 'dispatches' },
    ]
  },
  ...
  uber: {
    ...
    classes: [
      { name: 'UberService', methods: ['+ registerRider(rider): Rider', '+ registerDriver(driver): Driver', '+ estimate(pickup, dropoff, type): FareEstimate', '+ requestRide(userId, pickup, dropoff, type, fare, distance): Ride', '+ getAvailableRideRequestsForDriver(driverId): List<Ride>', '+ acceptRide(rideId, driverId): Ride', '+ declineRide(rideId, driverId): Ride', '+ verifyOtpAndStart(rideId, otp): Ride', '+ arriveAtDestination(rideId): Ride', '+ completeTrip(rideId, paymentMethod): Ride', '+ cancelTrip(rideId): Ride'] },
      ...
      { name: 'Ride', fields: ['- id: String', '- rider: Rider', '- driver: Driver', '- pickup: Location', '- dropoff: Location', '- distanceKm: double', '- fare: double', '- status: RideStatus', '- payment: Payment', '- otp: String', '- declinedDriverIds: Set<String>'], methods: ['+ verifyOtp(input): boolean', '+ addDeclinedDriver(driverId): void', '+ isDeclinedBy(driverId): boolean'] },
      ...
      { name: 'RideStatus', stereotype: 'enum', fields: ['REQUESTED', 'ACCEPTED', 'ONGOING', 'DESTINATION_REACHED', 'PAYMENT_PENDING', 'COMPLETED', 'PAYMENT_FAILED', 'CANCELLED'], methods: [] },
      { name: 'FareEstimate', stereotype: 'record', fields: ['- distanceKm: double', '- fare: double', '- estimatedMinutes: int', '- vehicleType: VehicleType'], methods: [] },
    ...
    relationships: [
      ...
      { from: 'UberService', to: 'FareEstimate', label: 'returns' },
  ...
  vendingmachine: {
    ...
    classes: [
      { name: 'VendingMachine', fields: ['- machineId: String', '- currentState: VendingMachineState', '- slots: Map<String, Slot>', '- changeInventory: Map<Denomination, AtomicInteger>', '- lock: ReentrantLock', '- changeDispenserChain: ChangeDispenserChain'], methods: ['+ selectProduct(slotCode): void', '+ insertMoney(denomination): void', '+ dispense(): Transaction', '+ cancelTransaction(): Transaction', '+ refillChange(denom, count): void', '+ restockSlot(code, qty): void'] },
      { name: 'VendingMachineState', stereotype: 'interface', fields: [], methods: ['+ selectProduct(machine, code): void', '+ insertMoney(machine, denom): void', '+ dispense(machine): Transaction', '+ cancelTransaction(machine): Transaction', '+ getStatus(): MachineStatus'] },
      { name: 'IdleState', fields: [], methods: ['+ selectProduct(machine, code): void', '+ insertMoney(machine, denom): void', '+ dispense(machine): Transaction', '+ cancelTransaction(machine): Transaction'] },
      { name: 'HasSelectionState', fields: [], methods: ['+ selectProduct(machine, code): void', '+ insertMoney(machine, denom): void', '+ dispense(machine): Transaction', '+ cancelTransaction(machine): Transaction'] },
      { name: 'HasMoneyState', fields: [], methods: ['+ selectProduct(machine, code): void', '+ insertMoney(machine, denom): void', '+ dispense(machine): Transaction', '+ cancelTransaction(machine): Transaction'] },
      { name: 'DispensingState', fields: [], methods: ['+ selectProduct(machine, code): void', '+ insertMoney(machine, denom): void', '+ dispense(machine): Transaction', '+ cancelTransaction(machine): Transaction'] },
      { name: 'ChangeDispenseHandler', stereotype: 'abstract', fields: ['# nextHandler: ChangeDispenseHandler', '# denomination: Denomination'], methods: ['+ setNext(handler): ChangeDispenseHandler', '+ handle(remaining, avail, dispensed): void'] },
      { name: 'DenominationChangeHandler', fields: [], methods: ['+ handle(remaining, avail, dispensed): void'] },
      { name: 'ChangeDispenserChain', fields: ['- chainHead: ChangeDispenseHandler'], methods: ['+ calculateChange(amount, available): Map<Denomination, Integer>'] },
      { name: 'Slot', fields: ['- code: String', '- row: int', '- col: int', '- product: Product', '- capacity: int', '- currentStock: int'], methods: ['+ isAvailable(): boolean', '+ decrementStock(): void', '+ restock(qty): void'] },
      { name: 'Product', fields: ['- id: long', '- code: String', '- name: String', '- price: double', '- category: String', '- emoji: String'], methods: [] },
      { name: 'Transaction', fields: ['- id: long', '- slotCode: String', '- itemPrice: double', '- insertedAmount: double', '- changeAmount: double', '- changeBreakdown: Map', '- status: String'], methods: [] },
      { name: 'Denomination', stereotype: 'enum', fields: ['COIN_1(1)', 'COIN_2(2)', 'COIN_5(5)', 'COIN_10(10)', 'NOTE_20(20)', 'NOTE_50(50)', 'NOTE_100(100)', 'NOTE_500(500)'], methods: ['+ getValue(): int', '+ getType(): String', '+ fromValue(val): Denomination'] },
      { name: 'VendingMachineService', stereotype: 'service', fields: ['- mainMachine: VendingMachine', '- simMachine: VendingMachine', '- simEvents: List<SimEvent>'], methods: ['+ getSlots(): List', '+ selectProduct(code): Transaction', '+ insertMoney(val): Transaction', '+ dispense(): Transaction', '+ simReset(): Map', '+ simDispense(step): Map'] },
    ],
    relationships: [
      { from: 'VendingMachine', to: 'VendingMachineState', label: 'has state' },
      { from: 'IdleState', to: 'VendingMachineState', label: 'implements', dashed: true },
      { from: 'HasSelectionState', to: 'VendingMachineState', label: 'implements', dashed: true },
      { from: 'HasMoneyState', to: 'VendingMachineState', label: 'implements', dashed: true },
      { from: 'DispensingState', to: 'VendingMachineState', label: 'implements', dashed: true },
      { from: 'VendingMachine', to: 'Slot', label: 'manages (12)' },
      { from: 'Slot', to: 'Product', label: 'holds' },
      { from: 'VendingMachine', to: 'ChangeDispenserChain', label: 'uses' },
      { from: 'ChangeDispenserChain', to: 'ChangeDispenseHandler', label: 'starts with' },
      { from: 'DenominationChangeHandler', to: 'ChangeDispenseHandler', label: 'extends' },
      { from: 'ChangeDispenseHandler', to: 'ChangeDispenseHandler', label: 'next (CoR)' },
      { from: 'VendingMachine', to: 'Transaction', label: 'creates / records' },
      { from: 'VendingMachineService', to: 'VendingMachine', label: 'manages' },
    ]
  },
  ...
  loggingFramework: {
    ...
    classes: [
      { name: 'LogManager', stereotype: 'singleton', fields: ['- loggers: Map<String, Logger>', '- rootLogger: Logger', '- activeFormatter: LogFormatter'], methods: ['+ getLogger(name): Logger', '+ setGlobalLevel(level): void', '+ setFormatter(formatter): void'] },
      { name: 'Logger', stereotype: 'entity', fields: ['- name: String', '- level: LogLevel', '- parent: Logger', '- appenders: List<LogAppender>', '- activeFormatter: LogFormatter'], methods: ['+ log(level, msg, ctx): LogMessage', '+ getEffectiveLevel(): LogLevel', '+ info(msg): LogMessage', '+ error(msg): LogMessage'] },
      { name: 'LogLevel', stereotype: 'enum', fields: ['TRACE(1)', 'DEBUG(2)', 'INFO(3)', 'WARN(4)', 'ERROR(5)', 'FATAL(6)'], methods: ['+ isGreaterOrEqual(threshold): boolean'] },
      { name: 'LogMessage', stereotype: 'model', fields: ['- id: Long', '- level: LogLevel', '- loggerName: String', '- message: String', '- threadName: String', '- timestamp: LocalDateTime', '- context: Map<String, Object>'], methods: [] },
      { name: 'LogHandler', stereotype: 'abstract', fields: ['# level: LogLevel', '# nextHandler: LogHandler'], methods: ['+ handle(msg, appenders, formatter, asyncDispatcher, isAsync): boolean', '+ setNext(nextHandler): LogHandler'] },
      { name: 'LogAppender', stereotype: 'interface', fields: [], methods: ['+ append(message, formatter): void', '+ getStatus(): AppenderStatus', '+ isEnabled(): boolean'] },
      { name: 'ConsoleAppender', fields: ['- name: String', '- enabled: boolean'], methods: ['+ append(message, formatter): void'] },
      { name: 'FileAppender', fields: ['- filePath: String', '- maxBytes: long', '- rotatedFiles: List<List<String>>'], methods: ['+ append(message, formatter): void', '- rotate(): void'] },
      { name: 'DatabaseAppender', fields: ['- dbRecords: List<String>'], methods: ['+ append(message, formatter): void'] },
      { name: 'ElasticsearchAppender', fields: ['- indexPrefix: String'], methods: ['+ append(message, formatter): void'] },
      { name: 'LogFormatter', stereotype: 'interface', fields: [], methods: ['+ format(message): String', '+ getType(): FormatterType'] },
      { name: 'SimpleTextFormatter', fields: [], methods: ['+ format(message): String'] },
      { name: 'JsonFormatter', fields: ['- objectMapper: ObjectMapper'], methods: ['+ format(message): String'] },
      { name: 'PatternFormatter', fields: ['- pattern: String'], methods: ['+ format(message): String'] },
      { name: 'AsyncLogDispatcher', stereotype: 'worker', fields: ['- queue: BlockingQueue<LogTask>', '- droppedCount: AtomicLong'], methods: ['+ dispatch(msg, appenders, formatter): boolean'] },
    ],
    relationships: [
      { from: 'LogManager', to: 'Logger', label: 'manages' },
      { from: 'Logger', to: 'Logger', label: 'parent' },
      { from: 'Logger', to: 'LogLevel', label: 'uses' },
      { from: 'Logger', to: 'LogMessage', label: 'produces' },
      { from: 'Logger', to: 'LogHandler', label: 'dispatches to' },
      { from: 'LogHandler', to: 'LogHandler', label: 'next' },
      { from: 'LogHandler', to: 'LogAppender', label: 'sends to' },
      { from: 'LogHandler', to: 'AsyncLogDispatcher', label: 'delegates to' },
      ...
      { from: 'DatabaseAppender', to: 'LogAppender', label: 'implements', dashed: true },
      { from: 'ElasticsearchAppender', to: 'LogAppender', label: 'implements', dashed: true },
      { from: 'SimpleTextFormatter', to: 'LogFormatter', label: 'implements', dashed: true },
      { from: 'JsonFormatter', to: 'LogFormatter', label: 'implements', dashed: true },
      { from: 'PatternFormatter', to: 'LogFormatter', label: 'implements', dashed: true },
    ]
  },
  ...
  lrucache: {
    ...
    classes: [
      { name: 'LruCacheService', stereotype: 'singleton', fields: ['- cache: LruCache<String, String>'], methods: ['+ get(key): String', '+ put(key, value): void', '+ remove(key): boolean', '+ clear(): void', '+ setCapacity(capacity): void', '+ setPolicy(type): void', '+ getSnapshot(): Map', '+ getStats(): Map', '+ batchSimulate(): Map'] },
      { name: 'LruCache', fields: ['- capacity: int', '- map: ConcurrentHashMap<K, Node<K,V>>', '- evictionPolicy: EvictionPolicy<K,V>', '- lock: ReentrantLock', '- totalHits: long', '- totalMisses: long', '- totalEvictions: long', '- logs: List<Map>'], methods: ['+ get(key): V', '+ put(key, value): void', '+ remove(key): boolean', '+ setCapacity(capacity): void', '+ setPolicy(type): void', '+ getSnapshot(): Map'] },
      { name: 'Node', fields: ['- key: K', '- value: V', '- prev: Node<K,V>', '- next: Node<K,V>', '- accessCount: int', '- createdAt: long', '- lastAccessedAt: long'], methods: ['+ incrementAccessCount(): void', '+ updateLastAccessedAt(): void'] },
      { name: 'EvictionPolicy', stereotype: 'interface', fields: [], methods: ['+ keyAccessed(node): void', '+ keyInserted(node): void', '+ evictKey(): Node', '+ removeKey(node): void', '+ getOrderedNodes(): List<Node>', '+ getType(): EvictionPolicyType'] },
      { name: 'LRUEvictionPolicy', fields: ['- head: Node<K,V>', '- tail: Node<K,V>'], methods: ['implements EvictionPolicy'] },
      { name: 'LFUEvictionPolicy', fields: ['- nodes: List<Node<K,V>>'], methods: ['implements EvictionPolicy'] },
      { name: 'FIFOEvictionPolicy', fields: ['- queue: List<Node<K,V>>'], methods: ['implements EvictionPolicy'] },
      { name: 'EvictionPolicyType', stereotype: 'enum', fields: ['LRU', 'LFU', 'FIFO'], methods: [] }
    ],
    relationships: [
      { from: 'LruCacheService', to: 'LruCache', label: 'delegates to' },
      { from: 'LruCache', to: 'Node', label: 'manages map & pointers' },
      { from: 'LruCache', to: 'EvictionPolicy', label: 'strategy pattern' },
      { from: 'LRUEvictionPolicy', to: 'EvictionPolicy', label: 'implements', dashed: true },
      { from: 'LFUEvictionPolicy', to: 'EvictionPolicy', label: 'implements', dashed: true },
      { from: 'FIFOEvictionPolicy', to: 'EvictionPolicy', label: 'implements', dashed: true },
  ...
  splitwise: {
    ...
    classes: [
      ...
      { name: 'ExpenseEvent', stereotype: 'dto', fields: ['- id: long', '- type: ExpenseEventType', '- actor: String', '- description: String', '- balanceSnapshot: Map', '- timestamp: LocalDateTime'], methods: [] },
      { name: 'ExpenseEventType', stereotype: 'enum', fields: ['USER_CREATED', 'GROUP_CREATED', 'MEMBER_ADDED', 'EXPENSE_ADDED', 'SETTLEMENT'], methods: [] },
  ...
  atm: {
    ...
    classes: [
      { name: 'AtmService', stereotype: 'singleton', fields: ['- bankingService: BankingService', '- cashDispenser: CashDispenser', '- currentState: ATMState', '- activeCard: Card', '- activeAccount: Account'], methods: ['+ insertCard(cardNumber): Map', '+ authenticate(cardNumber, pin): Account', '+ getBalance(accNum): double', '+ withdraw(accNum, amount): WithdrawalTransaction', '+ deposit(accNum, amount, notes): DepositTransaction', '+ ejectCard(): Map', '+ simReset()', '+ simWithdraw(...)'] },
      { name: 'CashDispenser', fields: ['- noteInventory: ConcurrentHashMap<NoteDenomination, Integer>', '- dispenserLock: ReentrantLock', '- dispenseStrategy: DenominationDispenseStrategy'], methods: ['+ getTotalCashAvailable(): int', '+ getInventory(): Map', '+ addNotes(denom, count): void', '+ setNoteCount(denom, count): void', '+ dispenseCash(amount): Map'] },
      { name: 'DenominationDispenseStrategy', stereotype: 'interface', fields: [], methods: ['+ calculateNotes(amount, availableInventory): Map'] },
      { name: 'GreedyDenominationDispenseStrategy', fields: ['implements DenominationDispenseStrategy'], methods: ['+ calculateNotes(amount, availableInventory): Map'] },
      { name: 'Account', fields: ['- id: String', '- accountNumber: String', '- holderName: String', '- balance: double', '- accountLock: ReentrantLock'], methods: ['+ getLock(): ReentrantLock', '+ getBalance(): double', '+ setBalance(b): void'] },
      { name: 'Card', fields: ['- cardNumber: String', '- pin: String', '- accountNumber: String', '- failedPinAttempts: AtomicInteger', '- isBlocked: boolean'], methods: ['+ incrementFailedAttempts(): int', '+ blockCard(): void'] },
      { name: 'ATMState', stereotype: 'enum', fields: ['IDLE', 'CARD_INSERTED', 'AUTHENTICATED', 'TRANSACTION_IN_PROGRESS', 'DISPENSING', 'SESSION_ENDED', 'CARD_BLOCKED'], methods: [] },
      { name: 'NoteDenomination', stereotype: 'enum', fields: ['TWO_THOUSAND(2000)', 'FIVE_HUNDRED(500)', 'TWO_HUNDRED(200)', 'ONE_HUNDRED(100)'], methods: ['+ getValue(): int'] },
      { name: 'Transaction', stereotype: 'abstract', fields: ['- transactionId: String', '- accountNumber: String', '- amount: double', '- timestampEpoch: long', '- status: String', '- failureReason: String'], methods: ['+ execute(bankingService, cashDispenser)'] },
      { name: 'WithdrawalTransaction', fields: ['extends Transaction', '- dispensedNotes: Map<NoteDenomination, Integer>'], methods: [] },
      { name: 'DepositTransaction', fields: ['extends Transaction', '- depositedNotes: Map<NoteDenomination, Integer>'], methods: [] }
    ],
    relationships: [
      { from: 'AtmService', to: 'CashDispenser', label: 'controls hardware dispenser' },
      { from: 'AtmService', to: 'BankingService', label: 'delegates account lookups' },
      { from: 'AtmService', to: 'ATMState', label: 'maintains session state' },
      { from: 'CashDispenser', to: 'DenominationDispenseStrategy', label: 'uses' },
      { from: 'GreedyDenominationDispenseStrategy', to: 'DenominationDispenseStrategy', label: 'implements', dashed: true },
      { from: 'WithdrawalTransaction', to: 'Transaction', label: 'extends' },
      { from: 'DepositTransaction', to: 'Transaction', label: 'extends' }
    ]
  },
  coffeemachine: {
    title: 'Coffee Vending Machine — Class Diagram',
    classes: [
      { name: 'CoffeeMachineService', stereotype: 'singleton', fields: ['- mainMachine: CoffeeMachine', '- simMachine: CoffeeMachine', '- simEvents: List<SimEvent>'], methods: ['+ getMenu(): List<CoffeeRecipe>', '+ getStatus(): Map', '+ startOrder(type): CoffeeOrder', '+ addCustomization(addOn): CoffeeOrder', '+ insertPayment(amount): CoffeeOrder', '+ brew(): CoffeeOrder', '+ collectCoffee(): CoffeeOrder', '+ cancelOrder(): CoffeeOrder', '+ simReset(): Map', '+ simSimulateRace(step): Map'] },
      { name: 'CoffeeMachine', fields: ['- machineId: String', '- ingredientStore: IngredientStore', '- coffeeFactory: CoffeeFactory', '- currentState: CoffeeMachineState', '- currentOrder: CoffeeOrder', '- activeComponent: CoffeeComponent', '- sessionLock: ReentrantLock', '- brewHeadLock: ReentrantLock'], methods: ['+ selectBaseCoffee(type)', '+ addCustomization(addOn)', '+ wrapWithDecorator(addOn)', '+ insertPayment(amount)', '+ brew()', '+ collectCoffee()', '+ cancel()'] },
      { name: 'CoffeeComponent', stereotype: 'interface', fields: [], methods: ['+ getDescription(): String', '+ getPrice(): double', '+ getRequiredIngredients(): Map<IngredientType, Integer>'] },
      { name: 'BaseCoffee', fields: ['implements CoffeeComponent', '- type: CoffeeType', '- description: String', '- price: double', '- baseIngredients: Map<IngredientType, Integer>'], methods: ['+ getDescription()', '+ getPrice()', '+ getRequiredIngredients()'] },
      { name: 'CoffeeDecorator', stereotype: 'abstract', fields: ['implements CoffeeComponent', '# decoratedCoffee: CoffeeComponent'], methods: ['+ getDescription()', '+ getPrice()', '+ getRequiredIngredients()', '# getAddedIngredients(): Map'] },
      { name: 'ExtraShotDecorator', fields: ['extends CoffeeDecorator', '- EXTRA_SHOT_PRICE: double'], methods: ['+ getDescription()', '+ getPrice()', '+ getAddedIngredients()'] },
      { name: 'WhippedCreamDecorator', fields: ['extends CoffeeDecorator', '- WHIPPED_CREAM_PRICE: double'], methods: ['+ getDescription()', '+ getPrice()', '+ getAddedIngredients()'] },
      { name: 'OatMilkDecorator', fields: ['extends CoffeeDecorator', '- OAT_MILK_PRICE: double'], methods: ['+ getDescription()', '+ getPrice()', '+ getRequiredIngredients()'] },
      { name: 'CoffeeFactory', stereotype: 'factory', fields: ['- recipeRegistry: ConcurrentHashMap<CoffeeType, CoffeeRecipe>'], methods: ['+ registerRecipe(recipe): void', '+ createBaseCoffee(type): CoffeeComponent', '+ getAllRecipes(): List<CoffeeRecipe>'] },
      { name: 'CoffeeRecipe', fields: ['- type: CoffeeType', '- name: String', '- description: String', '- basePrice: double', '- ingredients: Map<IngredientType, Integer>'], methods: ['+ getIngredients()'] },
      { name: 'IngredientStore', fields: ['- inventory: ConcurrentHashMap<IngredientType, AtomicInteger>', '- ingredientLocks: ConcurrentHashMap<IngredientType, ReentrantLock>', '- capacities: ConcurrentHashMap', '- lowStockThresholds: ConcurrentHashMap'], methods: ['+ refill(type, amount): void', '+ checkAvailability(req): boolean', '+ checkAndDeductIngredients(req): boolean', '+ getLowStockAlerts(): List'] },
      { name: 'CoffeeMachineState', stereotype: 'interface', fields: [], methods: ['+ selectBaseCoffee(m, type): void', '+ addCustomization(m, addOn): void', '+ insertPayment(m, amount): void', '+ brew(m): CoffeeOrder', '+ collectCoffee(m): CoffeeOrder', '+ cancel(m): CoffeeOrder'] },
      { name: 'IdleState', fields: ['implements CoffeeMachineState'], methods: ['+ selectBaseCoffee(m, type)'] },
      { name: 'SelectingState', fields: ['implements CoffeeMachineState'], methods: ['+ addCustomization(m, addOn)', '+ insertPayment(m, amount)'] },
      { name: 'PaymentPendingState', fields: ['implements CoffeeMachineState'], methods: ['+ insertPayment(m, amount)', '+ brew(m)', '+ cancel(m)'] },
      { name: 'BrewingState', fields: ['implements CoffeeMachineState'], methods: ['+ brew(m)'] },
      { name: 'DispensedState', fields: ['implements CoffeeMachineState'], methods: ['+ collectCoffee(m)'] },
      { name: 'CoffeeOrder', fields: ['- orderId: long', '- baseCoffeeName: String', '- description: String', '- totalPrice: double', '- amountPaid: double', '- changeReturned: double', '- status: String', '- customizations: List<String>'], methods: [] },
      { name: 'IngredientType', stereotype: 'enum', fields: ['WATER', 'MILK', 'COFFEE_BEANS', 'SUGAR', 'WHIPPED_CREAM', 'CARAMEL_SYRUP', 'OAT_MILK'], methods: ['+ getUnit(): String'] }
    ],
    relationships: [
      { from: 'CoffeeMachineService', to: 'CoffeeMachine', label: 'manages' },
      { from: 'CoffeeMachine', to: 'CoffeeComponent', label: 'wraps active composition' },
      { from: 'CoffeeMachine', to: 'CoffeeMachineState', label: 'delegates lifecycle transitions' },
      { from: 'CoffeeMachine', to: 'CoffeeFactory', label: 'queries recipes' },
      { from: 'CoffeeMachine', to: 'IngredientStore', label: 'checks & deducts stores' },
      { from: 'BaseCoffee', to: 'CoffeeComponent', label: 'implements', dashed: true },
      { from: 'CoffeeDecorator', to: 'CoffeeComponent', label: 'implements & wraps', dashed: true },
      { from: 'ExtraShotDecorator', to: 'CoffeeDecorator', label: 'extends' },
      { from: 'WhippedCreamDecorator', to: 'CoffeeDecorator', label: 'extends' },
      { from: 'OatMilkDecorator', to: 'CoffeeDecorator', label: 'extends' },
      { from: 'IdleState', to: 'CoffeeMachineState', label: 'implements', dashed: true },
      { from: 'SelectingState', to: 'CoffeeMachineState', label: 'implements', dashed: true },
      { from: 'PaymentPendingState', to: 'CoffeeMachineState', label: 'implements', dashed: true },
      { from: 'BrewingState', to: 'CoffeeMachineState', label: 'implements', dashed: true },
      { from: 'DispensedState', to: 'CoffeeMachineState', label: 'implements', dashed: true }
    ]
  }
};

export const ATM_DIAGRAM = classDiagrams.atm;
